...
class SimpleButton extends Button {
  static styles = [
    ...
    `background: linear-gradient(-45deg, #003c8f, #1976d2)`,
    `color: #fafafa`,
...
export class CounterView extends StatefulWidget {
  ...
  build() {
    return new SpaceEvenly(
        new ExtraLargeText(this.state.count).setStyles(`user-select: none`),
...
export default new HTMLWidget(
    ...
    new BodyWidget(
        ...
        new Expanded(
            ...
            `background: linear-gradient(to top left, #003c8f, #bbdefb)`,
            `padding: 2rem`,
        ),
        new PageView(
            new Heading1(
