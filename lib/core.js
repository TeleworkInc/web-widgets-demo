...
export class Widget extends Inheritable {
  ...
  /**
   * Apply all of this widget's metadata to the DOM node.
   *
   * @return {Widget|StatefulWidget} this
   */
  applyMetadata() {
  ...
  constructElement() {
    /**
     * Export all inheritable class styles to the global namespace. This will
     * collapse to noop if we're compiling a namespace in release mode (in which
     * case, the styles will be hydrated from the DOM node's `styles` attr).
     */
    this.constructor.exportStyles();

    /**
     * To construct the Widget element, create a new DOM node and apply all of
